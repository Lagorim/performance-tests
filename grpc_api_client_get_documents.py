...
acconts_gateway_client = build_accounts_gateway_grpc_client()
...
# Шаг 2. Открыть кредитный счет
open_credit_card_account_response = acconts_gateway_client.open_credit_card_account(
    user_id=create_user_response.user.id
)
print('Open credit card account response:', open_credit_card_account_response)

# Шаг 3. Получить документ тарифа
get_tarrif_document_response = documents_gateway_client.get_tariff_document(
    account_id=open_credit_card_account_response.account.id
)
print('Get tarrif document response:', get_tarrif_document_response)

#Шаг 4. Получить документ контракта
